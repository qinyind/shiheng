...
3. AI 识餐默认通过 Docker 内网连接 CPA（`http://cli-proxy-api:8317/v1`），使用 `gpt-5.6-luna`。填写专用于食衡的 `AI_API_KEY`，不要复用 CPA 管理密钥；CPA 网络名可通过 `CPA_NETWORK_NAME` 调整。CPA 的 API、管理面板和 OAuth 回调端口应只绑定宿主机回环地址，不要暴露到公网。
